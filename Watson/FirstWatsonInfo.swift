...
struct FirstWatsonInfo: View {
    ...
    var body: some View {
        ...
        if(self.tutorial == 1 && self.timeRemaining == 0)
        {
            SUTutorial()
        }
        else{
    ...
    else if (inputBar == 3 && self.timeRemaining < 2)
    {
        ScrollView(.vertical, showsIndicators: false)
        {
            VStack{
                ...
                {
                Button (action: {
                    self.timeRemaining = 2; self.t5 = 1; self.tutorial = 1;
                }){
...
struct FirstWatsonInfo_Previews: PreviewProvider {
